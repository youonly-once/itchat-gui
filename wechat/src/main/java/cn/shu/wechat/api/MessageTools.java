...
import cn.shu.wechat.utils.enums.*;
...
public class MessageTools {
        ...
        for (Result result : results) {
            ...
            switch (result.replyMsgTypeEnum) {
                ...
                case VIDEO:
                    ...
                    break;
                default://其他消息发送文件
                    sendFileMsgByUserId(toUserId, result.msg);
            }
    ...
    public static SendMsgResponse sendTextMsgByUserId(String content, String toUserName) {
        ...
        msgMap.put("Type", ReplyMsgTypeEnum.TEXT.getCode());
            ...
            try {
                ...
                for(; sliceIndex < sliceCount; ++sliceIndex) {
                 ...
                 //   builder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
                    ...
                    builder.addBinaryBody("filename",copyOfRange,ContentType.create(mimeType), filePath);
    ...
    static JSONObject webwxuploadmedia( String  filePath,String fromUserName,String toUserName) throws IOException {
        ...
        if (file.getName().toLowerCase().endsWith(".mp4")) {
            int bitRate = 800000;
            while (fileSize > 1024 * 1024) {
                file = MediaUtil.compressionVideo(file, "/compression/" + file.getName() + ".mp4", bitRate);
                fileSize = file.length();
                bitRate = (int) (bitRate / 2);
            }
        }
        if ("pic".equals(fileType) && fileSize > 1024 * 1024) {
        ...
        if (file.length() < 1048576L) {
            ...
            request.content("lastModifiedDate", lastModifieDate);
            ...
            BufferedInputStream bfinStream = new BufferedInputStream(new FileInputStream(file));
    ...
    public static SendMsgResponse sendPicMsgByUserId(String userId, String filePath) {
        ...
        if (responseObj != null) {
            ...
            Map<String, Object> msgMap = new HashMap<String, Object>();
            msgMap.put("Type", ReplyMsgTypeEnum.PIC.getCode());
    ...
    public static SendMsgResponse sendVideoMsgByUserId(String userId, String filePath) {
        ...
        if (responseObj != null) {
            ...
            String url = String.format("%s/webwxsendvideomsg?fun=async&f=json&lang=zh_CN&pass_ticket=%s", Core.getLoginInfoMap().get("url"),
                    Core.getLoginInfoMap().get("pass_ticket"));
            ...
            msgMap.put("MediaId", mediaId);
            msgMap.put("ToUserName", userId);
            msgMap.put("Type", MsgCodeEnum.MSGTYPE_VIDEO.getCode());
            Map<String, Object> paramMap = Core.getParamMap();
            paramMap.put("BaseRequest", Core.getParamMap().get("BaseRequest"));
            paramMap.put("Msg", msgMap);
            String paramStr = JSON.toJSONString(paramMap);
    ...
    public static SendMsgResponse sendFileMsgByUserId(String userId, String filePath) {
        ...
        data.put("totallen", "");
        data.put("attachid", "");
        data.put("type", ReplyMsgTypeEnum.APP.getCode() + ""); // APPMSGTYPE_ATTACH
