...
//						log.info(JSONObject.toJSONString(resultMap));
                        ...
                        if (retcode.equals(RetCodeEnum.UNKOWN.getCode())) {
                            ...
                            if (webWxSyncMsg.getModContactCount() > 0) {
                                System.out.println(webWxSyncMsg.getModContactList());
                            }
                            if (selector.equals("2")) {
                                if (webWxSyncMsg != null) {
                                    try {
                                        ...
                                        //处理聊天室修改消息
                                        //处理联系人删除消息
                                        MsgCenter.produceDelContactMsg(webWxSyncMsg.getDelContactList());
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                        log.info(e.getMessage());
                                    }
                                ...
                                try {
                                    List<AddMsgList> addMsgList = webWxSyncMsg.getAddMsgList();
                                    List<ModContactList> modContactList = webWxSyncMsg.getModContactList();// 存在删除或者新增的好友信息
                                    MsgCenter.produceNewMsg(addMsgList);
                                    for (ModContactList contactList : modContactList) {
                                        // 存在主动加好友之后的同步联系人到本地
                                        core.getContactMap().put(contactList.getUserName(), JSON.parseObject(JSON.toJSONString(contactList)));
                                    }
                                } catch (Exception e) {
...
//			log.info("core.getGroupMemeberMap()："+core.getGroupMemberMap().size());
    ...
    private WebWxSyncMsg webWxSync() {
        WebWxSyncMsg webWxSyncMsg = null;
        ...
        try {
            ...
            String text = EntityUtils.toString(entity, Consts.UTF_8);
            webWxSyncMsg = JSON.parseObject(text, WebWxSyncMsg.class);
            //JSONObject obj = JSON.parseObject(text);
            if (webWxSyncMsg.getBaseResponse().getRet() == 0) {
                ...
                List<cn.shu.wechat.beans.sync.List> lists = webWxSyncMsg.getSyncKey().getList();
                StringBuilder sb = new StringBuilder();
                for (cn.shu.wechat.beans.sync.List list : lists) {
